    ...
    spec: {
      ...
      template: {
        ...
        spec: {
          ...
          securityContext: { fsGroup: 10001, seccompProfile: { type: "RuntimeDefault" } },
          initContainers: [
            // The source is an exact digest from Forge's private registry. ORAS still
            // rejects the absolute, mounted output directory unless this is explicit.
            { name: "fetch-artifact", image: image(orasImage, "INVALID_ORAS_IMAGE"), imagePullPolicy: "Never", command: ["oras", "pull", "--plain-http", "--allow-path-traversal", artifact, "--output", "/download"], securityContext: securityContext(), resources: resources({ cpu: "100m", memory: "128Mi", storage: "512Mi" }), volumeMounts: [{ name: "download", mountPath: "/download" }, { name: "tmp", mountPath: "/tmp" }, { name: "home", mountPath: "/home/forge" }] },
