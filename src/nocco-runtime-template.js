...
export function createNoccoArtifactJob({ policy, buildPolicy, releaseId, commitSha, checkoutImage, builderImage, publisherImage, registryOrigin }) {
  ...
  const publishScript = "set -eu\noras push --plain-http \"$FORGE_ARTIFACT_REFERENCE\" /staging/app.tar:application/vnd.lyra.forge.nextjs.v1.tar";
  const checkoutEnv = [
    ...
    spec: {
      ...
      template: {
        ...
        spec: {
          ...
          securityContext: { fsGroup: 10001, seccompProfile: { type: "RuntimeDefault" } },
          initContainers: [
            { name: "fetch-artifact", image: image(orasImage, "INVALID_ORAS_IMAGE"), imagePullPolicy: "Never", command: ["/bin/sh", "-ec"], args: ["set -eu\noras pull --plain-http \"$FORGE_ARTIFACT_REFERENCE\" --output /download"], env: [{ name: "FORGE_ARTIFACT_REFERENCE", value: artifact }], securityContext: securityContext(), resources: resources({ cpu: "100m", memory: "128Mi", storage: "512Mi" }), volumeMounts: [{ name: "download", mountPath: "/download" }, { name: "tmp", mountPath: "/tmp" }, { name: "home", mountPath: "/home/forge" }] },
